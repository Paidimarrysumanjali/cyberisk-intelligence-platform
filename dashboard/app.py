...
import time, os, sys
import pandas as pd
import requests as _req # Renamed to avoid conflict with `requests` in scanner.py
from datetime import datetime
from dotenv import load_dotenv # Import load_dotenv
load_dotenv() # Load environment variables from .env

# Add the parent directory of 'modules' to Python path for imports to work
...
# Initial targets can be set in .env or default to scanme.nmap.org
INITIAL_TARGETS_STR = os.environ.get('SCAN_TARGETS', 'scanme.nmap.org')
print("\n--- DEBUGGING EMAIL CREDENTIALS ---")
print(f"GMAIL_SENDER from os.environ: '{GMAIL_SENDER}' (Length: {len(GMAIL_SENDER)})")
print(f"GMAIL_PASSWORD from os.environ: '{GMAIL_PASSWORD}' (Length: {len(GMAIL_PASSWORD)})")
print(f"GMAIL_RECIPIENT from os.environ: '{GMAIL_RECIPIENT}' (Length: {len(GMAIL_RECIPIENT)})")
email_ready_check = bool(GMAIL_SENDER and GMAIL_PASSWORD and GMAIL_RECIPIENT)
print(f"Email ready check (bool(GMAIL_SENDER and GMAIL_PASSWORD and GMAIL_RECIPIENT)): {email_ready_check}")
print("-----------------------------------\n")


# ── Session State Initialization ──────────────────────────────────────────────
# Streamlit's way to persist variables across reruns.
for key, val in [
    ('df', None),
    ('scan_time', None),
    ('last_refreshed', None),
    ('selected_scan_id', None),
    ('scan_targets_input', INITIAL_TARGETS_STR) # Initialize target input
]:
...
st.sidebar.markdown('**_Network Reconnaissance & Threat Intelligence_**')
st.sidebar.divider()
...
# Scan Target Input in Sidebar
st.sidebar.subheader('🎯 Scan Targets')
# Use a key to ensure this widget maintains its state
...
# Update session state with the latest input for persistence
...
scan_btn    = st.sidebar.button('Run Full Scan',  use_container_width=True, type='primary')
clear_btn = st.sidebar.button('Clear Current Scan',   use_container_width=True)

st.sidebar.divider()

# ── Header ────────────────────────────────────────────────────────────────────
st.title('👋 Welcome to CyberScan Pro')
st.caption('Your integrated platform for network security analysis.')
...
if scan_btn or clear_btn:
    # Clear existing session state data if clearing or starting new scan
    if clear_btn:
        st.session_state.df = None
        st.session_state.scan_time = None
        st.session_state.last_refreshed = None
        st.session_state.selected_scan_id = None # Also clear selected history scan
        st.rerun() # Rerun to clear UI immediately

    # Proceed with scan only if VT_KEY is available and targets are defined
    if not VT_KEY:
    ...
    all_nmap_rows = []
    # Adjust total_steps estimation for more accurate progress bar
    total_steps = len(current_targets_list) * 2 # Nmap scans + VT checks (approx)
    ...
    for i, target in enumerate(current_targets_list):
        ...
        if xml_file: # Only parse if Nmap produced output
    ...
    if df_raw.empty or "ip" not in df_raw.columns:
        ...
        st.session_state.df = pd.DataFrame() # Set to empty DF to avoid errors in Analysis page
        st.stop()
    ...
    # Step 5: Send Email Report (ALWAYS send, regardless of severity)
    ...
    if GMAIL_SENDER and GMAIL_PASSWORD and GMAIL_RECIPIENT:
        email_result = send_alert_email(
            ...
            GMAIL_PASSWORD,
            GMAIL_RECIPIENT,
            df_enriched, # Pass full dataframe, emailer filters internally
            st.session_state.scan_time,
            scan_summary_for_email # Pass the generated summary
        )
    ...
    # Step 6: Push to FastAPI API (for external access)
    ...
    try:
        _req.post(
            ...
            timeout=5 # Short timeout, API should be local
        )
    ...
    bar.empty() # Hide progress bar
    status.success(f'Scan complete! Found {len(df_enriched)} ports across {df_enriched["ip"].nunique()} hosts.')
    st.rerun() # Rerun to update the main dashboard with new data

# ── Main Dashboard Summary Display ──────────────────────────────────────────
# This section renders when a scan result (`st.session_state.df`) is available.
